...
COMPANY = 'Ritivel'
REPO = 'regulus-eval'
PROJECT_TERMS = ['product', 'promises', 'level', 'traceability', 'linking', 'statement', 'source', 'documents']
PROJECT_METRICS = ['product_coverage', 'promises_risk', 'level_precision', 'traceability_latency']
PROJECT_FAILURES = ['product_drift', 'promises_gap', 'level_misroute', 'traceability_blindspot']
PROJECT_ARCHETYPES = [{'name': 'product evidence replay', 'trigger': 'product signal changes while promises context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'promises boundary probe', 'trigger': 'promises handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'level regression harness', 'trigger': 'level behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'traceability operator packet', 'trigger': 'traceability output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = 'A regulatory document evaluation harness + citation integrity runtime for AI drafted FDA submissions. Quantifies hallucination, orphan citation, and edit decay rates — and produces the GxP evidence pack a pharma CISO will actually sign off on.'
...
HERO_TITLE = 'Ritivel Adversarial Replay Board'
